...
// Compare two doubles
#define DBL_CMP(lhs, rhs) (fabs((lhs) - (rhs)) < DBL_EPSILON)
...
// Asserts that the next token is a numeric literal with @expected as the value,
// and the token after that is an EOF
#define ASSERT_NUMBER(lx, string, expected) {       \
    lexer_init(&lx, string, strlen(string));        \
    struct token *tk = lexer_next(&lx);             \
    test_assert(tk->type == TOKEN_NUMERIC_LITERAL); \
    test_assert(DBL_CMP(tk->number, expected));     \
    test_assert(lexer_next(&lx)->type == TOKEN_EOF); }
...
test_case(lexer_next_numeric_literal)
{
    struct lexer lx;

    // Binary
    ASSERT_NUMBER(lx, "0b0", 0.0);
    ASSERT_NUMBER(lx, "-0b0", -0.0);
    ASSERT_NUMBER(lx, "0b10", 2.0);
    ASSERT_NUMBER(lx, "-0b10", -2.0);
    ASSERT_NUMBER(lx, "0B1010", 10.0);
    ASSERT_NUMBER(lx, "-0B1010", -10.0);
    ASSERT_NUMBER(lx, "0b1_0", 2.0);
    ASSERT_NUMBER(lx, "-0b1_0", -2.0);

    // ASSERT_NEXT(lx, "", TOKEN_INVALID);

    // Octal
    ASSERT_NUMBER(lx, "00", 0.0);
    ASSERT_NUMBER(lx, "-00", -0.0);
    ASSERT_NUMBER(lx, "0o33", 27.0);
    ASSERT_NUMBER(lx, "-0o33", -27.0);
    ASSERT_NUMBER(lx, "0O34", 28.0);
    ASSERT_NUMBER(lx, "-0O34", -28.0);
    ASSERT_NUMBER(lx, "0o3_5", 29.0);
    ASSERT_NUMBER(lx, "-0o3_5", -29.0);

    // Decimal
    ASSERT_NUMBER(lx, "37", 37.0);
    ASSERT_NUMBER(lx, "-37", -37.0);
    ASSERT_NUMBER(lx, "9.25", 9.25);
    ASSERT_NUMBER(lx, "-9.25", -9.25);

    // Hexadecimal
    ASSERT_NUMBER(lx, "0xff", 255.0);
    ASSERT_NUMBER(lx, "0XFF", 255.0);
