...
static int read_digits_helper(const char *bytes, double *value,
    const double base, bool (*is_fn)(const char), int (*to_fn)(const char))
{
    int i;
    char prev;

    // No underscores at the start
    if (bytes[0] == '_')
        return -1;
...
static int read_binary_digits(const char *bytes, double *value)
{
    return read_digits_helper(bytes, value, 2.0, isbin, bintoi);
}

static int read_octal_digits(const char *bytes, double *value)
{
    return read_digits_helper(bytes, value, 8.0, isoct, octtoi);
}

static int read_hex_digits(const char *bytes, double *value)
{
    return read_digits_helper(bytes, value, 16.0, ishex, hextoi);
}

static int read_decimal_digits(const char *bytes, double *value)
{
    return read_digits_helper(bytes, value, 10.0, isdec, dectoi);
}
...
//   DecimalBigIntegerLiteral ::
//     0 n
//     NonZeroDigit DecimalDigits n
//     NonZeroDigit _ DecimalDigits n
//
//   NonDecimalLiteral ::
//     BinaryIntegerLiteral
//     OctalIntegerLiteral
//     HexIntegerLiteral
static int parse_numeric_literal(struct lexer *lx)
{
    double value;
    int t;

    if (peek(lx) != '0')
        goto bigint_or_decimal;
    ...
    case 'b':
    case 'B':
        if ((t = read_binary_digits(lexer_position(lx), &value)) < 1)
            return -1;

        lx->index += t;
        goto end;

    case '0' ... '9': // Legacy notation
        lx->index--;
    case 'o':
    case 'O':
        if ((t = read_octal_digits(lexer_position(lx), &value)) < 1)
            return -1;
    ...
    case 'x':
    case 'X':
        if ((t = read_hex_digits(lexer_position(lx), &value)) < 1)
            return -1;
...
bigint_or_decimal:
    ...
    // if (isdecimal(peek(lx)) || (is_identifier_start(lexer_position(lx), NULL) != 0))
    //     return -1;

    token_set_numeric_literal(&lx->current, value);
    return 0;
}
...
struct token *lexer_next(struct lexer *lx)
{
    ...
    case '0' ... '9':
        lx->index--;
        // parse_bigint_or_numeric_literal(lx);
        break;

    case '"':
    case '\'':
        lx->index--;
        // parse_string_literal(lx);
        break;

    case '`':
        lx->index--;
        // parse_template_literal(lx);
        break;
